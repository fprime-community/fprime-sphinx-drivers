...
struct spwregs core0;

// STUB: ADDED, RESET VALUES FOR CORE0
static void stub_reset_core0(void)
{
  core0.ctrl = core0default.ctrl;
  core0.status = core0default.status;
  core0.nodeaddr = core0default.nodeaddr;
  core0.clkdiv = core0default.clkdiv;
  core0.destkey = core0default.destkey;
  core0.timereg = core0default.timereg;
}

static void stub_reset_core0ch0(void)
{
  core0.dma[0].ctrl = core0default.dma[0].ctrl;
  core0.dma[0].rxmaxlen = core0default.dma[0].rxmaxlen;
  core0.dma[0].txdesc = core0default.dma[0].txdesc;
  core0.dma[0].rxdesc = core0default.dma[0].rxdesc;
  core0.dma[0].addr = core0default.dma[0].addr;
}

static inline int loadmem(long addr)
{
  volatile int* ptr = (volatile int*)(long)addr;
  return *ptr;
}

static long ptrToInt(volatile const void* ptr)
{
  long ret = (long)ptr;
  return ret;
}
                 ...
                 int clkdivs)
{
	...
	spw->port = port;
        ...
        spw->regs = (struct spwregs *) (long)(&core0);// STUB: ADDED
        return 0;
}
                     ...
                     struct spwvars *spw)
{
        ...
        spw->dma[dmachan].txd = (struct txdescriptor *)(((long)txdescpnt & 0xffffffffL) | (((long)&txdescpnt) & ~(0xffffffffL))); // STUB: ADDED TO RESTORE UPPER ADDR BITS
        spw->dma[dmachan].rxd = (struct rxdescriptor *)(((long)rxdescpnt & 0xffffffffL) | (((long)&rxdescpnt) & ~(0xffffffffL))); // STUB: ADDED TO RESTORE UPPER ADDR BITS
        return 0;
}
...
int spw_init(struct spwvars *spw)
{
  stub_reset_core0(); // STUB: ADDED
        ...
        spw->regs->ctrl = (1 << 6); 
        stub_reset_core0(); // STUB: ADDED TO SIMULATE RESET
        tmp = loadmem(ptrToInt(&(spw->regs->ctrl)));
        ...
        for(i = 0; i < spw->dmachan; i++) {
          ...
          stub_reset_core0ch0(); // STUB: ADDED TO SIMULATE RESET
        ...
        //spw->regs->status = 0xFFF; /*clear status*/ // STUB: COMMENTED OUT FOR VERSION BELOW
        spw->regs->status &= ~0xFFF;
